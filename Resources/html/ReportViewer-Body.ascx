...
<div class="refresh-toolbar" style="position: relative; float:left; z-index: 6; margin: 12px 0 0 8px; white-space: nowrap;">
	<label for="refreshReportIntervals" style="font-size: 13px;">Refresh Every:</label>
	<select id="refreshReportIntervals" style="font-size: 13px; width: 120px; margin-left: 5px;"></select>
	<script type="text/javascript">
		<% foreach (KeyValuePair<string, int> interval in ((global_asax.CustomAdHocConfig) AdHocSettings.AdHocConfig).GetUpdateIntervalsInSeconds())
		{ %>
			jq$('#refreshReportIntervals').append(jq$("<option></option>").attr("value", "<%=interval.Value%>").text("<%=interval.Key%>"));
		<% } %>
	</script>
</div>
        ...
        <div id="fieldPropertiesDialogContainer">
            <div id="data-source-field" title="Field name" lang-title="js_FieldName">
	            <div id="propertiesDiv">
		            <div id="titleDiv" style="margin: 0px; text-align:left;text-transform:capitalize;color:#000000;background-color:#CCEEFF;padding:6px; width: 100%; max-width: 388px;"></div>
		            <div>

...
(function ($) {
	...
	*/
	...
	IzendaQuery.prototype = {
			...
			this.rsQuery({
				...
				wsArgs: [dashboardFullName],
					...
					if (data == null || data.Rows == null || data.Rows.length == 0) {
						dashboardLayout = {
							tiles: [$.extend({}, $.fn.izendaDashboard.tileDefaults, {
								isNew: true,
								width: 12,
								height: 4
							})]
		...
		* Load report
		*/
		loadReport: function (context, urlSettings, tile, width, height, doneHandler) {
			this.rsQuery({
				dataType: 'html',
				contextObject: context,
				wsCmd: 'getreportpartpreview',
				wsArgs: [urlSettings.reportInfo.fullName, tile.reportFullName, 1, tile.top, width, height - 35, 'forceSize'],
		...
		*/
		loadReportAndUpdate: function (context, urlSettings, tile, width, height, doneHandler) {
			this.rsQuery({
				dataType: 'html',
				contextObject: context,
		...
		*/
		...
		setDashboardToCrs: function(context, reportFullName, doneHandler) {
			this.rsQuery({
				contextObject: context,
				wsCmd: 'setdashboardtocrs',
				wsArgs: [reportFullName],
				done: function (data) {
					if (typeof doneHandler == 'function') {
						doneHandler.apply(this, [data]);

...
<script RunAt="server">
  ...
  public class CustomAdHocConfig : FileSystemAdHocConfig {
    ...
    //Indicates whether the ProcessDataSet method is in its first pass or second
    ...
    public override void ProcessDataSet(System.Data.DataSet ds, string reportPart) {
      // firstPass is needed to prevent delta list from being wiped out when the subtotal logic
      // loops back through ProcessDataSet.
      if (firstPass) {       
        firstPass = false;
          ...
          foreach (System.Data.DataTable table in ds.Tables) {
            ...
            foreach (string head in dataHeaders.Keys) {
              ...
              foreach (object column in dataColumns) {
                data.Add(table.Compute("Sum(" + dataColumns[i++].ToString() + ")", dataHeaders[head]));
              }
        ...
        //Reset firstPass so any future updates render correctly.
        firstPass = true;
      }
    }
    ...
    public override string PerformCustomRendering(string initialHtml) {
      ...
      foreach (HtmlNode delta in highestLevelDeltas) {
        ...
        string deltaHead = delta.InnerText.Replace(", ,", ",");
